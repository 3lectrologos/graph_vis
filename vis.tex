...
\usepackage{tikz}
\usetikzlibrary{decorations,positioning,fadings,through,spy}
...
\def\inputname{ComplexDeepFinal}
\def\xstretch{22}
...
\definecolor{col0}{HTML}{7b3294}
\definecolor{col1}{HTML}{0571b0}
\definecolor{col2}{HTML}{ca0020}
\definecolor{col3}{HTML}{008837}
...
\begin{tikzpicture}[
  ...
  \node[circle,draw=darkgray,fill=lightgray!50!white,line width=1pt,anchor=center,minimum size=45pt] (foo) at (-1.3, \ystretch-2) {\LARGE Epochs};
  ...
  \foreach \row in {0,...,\trows} {
	...
	\node[circle,draw=darkgray,fill=lightgray!50!white,line width=1pt,anchor=center,minimum size=45pt] (foo\row) at (-1.3, 2-\ystretch*\row) {\timetext{\time}};
  }
