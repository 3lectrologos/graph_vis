...
\def\inputname{HeFlatMiddle2Sign}
\def\xstretch{30cm}
\def\yoff{-0.8}
\def\ystretch{5}
\def\minnodesize{5}
...
\begin{tikzpicture}[
  ...
  \foreach \row in {0,...,\trows} {
	...
	\node[anchor=east] (foo\row) at (-0.5,\yoff -\ystretch*\row) {\timetext{\time}};
  }
  ...
  % xmin and xmax need to be the same as in Python file
  \def\xmin{-2.5}
  \def\xmax{2.5}
  % xlo and xhi are the desired limits
  \def\xlo{-1.5}
  \def\xhi{1.5}

  \foreach \rowi in {0,...,\trows} {
  \pgfmathsetmacro\row{int(\rowi + 1)}
  \pgfmathsetmacro\xat{(\xlo-\xmin)*30/(\xmax-\xmin)}
  \pgfmathsetmacro\xwidth{(\xhi-\xlo)*30/(\xmax-\xmin)}
  \begin{axis}[%
    anchor=south west,
    at={(\xat*1cm,-\row*5cm + 4.5cm)},
    axis y line=none,
    axis line style={
    	Goldenrod!80!black,
    	line width=2pt
    },
    axis background/.style={fill=Goldenrod!5!white, opacity=0.75},
    width=\xwidth*1cm,
    height=3.5cm,
    xmin=\xlo,
    xmax=\xhi,
