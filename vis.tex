...
\def\inputname{HeDeepLeftSign40}
\def\xstretch{22}
...
\begin{tikzpicture}[
  ...
  \foreach \row in {0,...,\trows} {
	...
	\node[circle,draw=darkgray!70!white,fill=lightgray!50!white,line width=1pt,anchor=center,minimum size=40pt] (foo\row) at (-1.5, 2.15-\ystretch*\row) {\timetext{\time}};
  }
  ...
  % epoch lines
  \def\tipwl{8pt}
  \node[draw=none, below=2cm of foo0] (afterfoo0) {};
  ...
  \foreach \row in {1,...,\trows} {
    \pgfmathsetmacro\prevrow{int(\row-1)}
  	\node[draw=none, above=1.7cm of foo\row] (beforefoo\row) {};
  	\node[draw=none, below=1.7cm of foo\row] (afterfoo\row) {};
